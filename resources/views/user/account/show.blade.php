    ...
    <div class="container-fluid">
        <div class="row">
            <div class="col-md-10 col-sm-offset-1">
                {!! Breadcrumbs::render('client.account.show', $conta) !!}
            </div>
        </div>
                                ...
                                <div class="panel-body">
                                    ...
                                    <div class="row">
                                        <div class="col-sm-6 col-xs-6">
                                            <form method="POST" action="{{route('client.payment.update', [$conta->idConta, $conta->pagamentos()->orderBy('created_at', 'desc')->first()->idPagamento])}}">
                                                {{csrf_field()}}
                                                {{method_field('PUT')}}
                                        ...
                                        <div class="col-sm-6 col-xs-6 text-right">
                                            <a class="btn btn-sm btn-primary" href="{{route('client.payment.index', $conta->idConta)}}">Ver todos</a>
                                        </div>
                                    </div>
                                </div>
                            </div>
                    ...
                    <div class="col-sm-6 col-sm-offset-3 tex-center">
                        <div class="form-group">
                            <a class="btn btn-lg btn-block btn-success {{(!count($conta->pagamentos) || $conta->prox_pagamento->subDay(5)->isToday() || $conta->prox_pagamento->subDay(5)->isPast())? ''  : 'disabled'}}"
                               href="{{route('client.payment.create', $conta->idConta)}}">
                                Gerar Novo Boleto
                            </a>
                        </div>
                    </div>
